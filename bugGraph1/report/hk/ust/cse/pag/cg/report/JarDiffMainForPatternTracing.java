...
/**
 * This program reads all jars in the directory and generate graph and explanations for each 
 * CFG with methods' name and version info
 * which is used for explanation of all the patterns 
 * 
 * There is advanced version, which compares only mapping CFGs: CFGbug <-->CFGfix
 * can use JarDiffMainOnlyMappingCFG.java to generate result for Hong's tool
 * and use JarDiffMainOnlyMappingCFGExpl.java to generate graph with explanation
 * and use Pattern2Source.java for tracing back the pattern to source info
 * 
 * @author 
 * 
 */
public class JarDiffMainForPatternTracing {
	...
	private static void getJarDiffGraph(
		...
		File fixGFile = new File(checkOutDir, "fix_graph_expl.txt");
		File bugGFile = new File(checkOutDir, "bug_graph_expl.txt");
		File nodeGFile = new File(checkOutDir, "graph_node_expl.txt");
		...
		while (true) {
			...
			// A mistake: according to "fix_rev.txt"'s content: 
			//"16904:16903 
			...
			JarDiffMainForPatternTracing diff = new JarDiffMainForPatternTracing();
