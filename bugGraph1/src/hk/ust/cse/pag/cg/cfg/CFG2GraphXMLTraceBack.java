 ...
 * 
 */
public class CFG2GraphXMLTraceBack {
	...
	public static void toGraphXML(CFGDiffResult result, PrintStream psBug,
		...
		if (isSimple) {
			...
			toGraphTxt(result,result.prevColoredNodes, "prev", "b", psBug,
					allNodeTable);
			toGraphTxt(result,result.newColoredNodes, "new", "f", psFix, allNodeTable);
			...
			//another bug here, however, this complex version is never used, so no problem
			//toGraphXML(result.newColoredNodes, "new", "#00FF00", psBug);
			toGraphXML(result.newColoredNodes, "new", "#00FF00", psFix);
	...
	private static void toGraphTxt(CFGDiffResult result, Set<Node> nodes, String prefix,
		...
		if (diffColor.equals("b")) {
			...
			String[] version=result.getPreVersion().toString().trim().split("_");
			
			versionNum=version[version.length-1];
			ps.println("t # " + (bugCount++)+";;"+" version "+versionNum+", class "+result.getPreClass().getName()+", method "+result.getMethodName());
		} else {
			String versionNum= new String();
			String[] version=result.getNewVersion().toString().trim().split("_");
			versionNum=version[version.length-1];
			ps.println("t # " + (fixCount++)+";;"+" version "+versionNum+", class "+result.getNewClass().getName()+", method "+result.getMethodName());
		}
   ...
   */
		...
		for (Node n : nodes) {
			if (n.color == 0 && !shownNodes.contains(n.id)) { // if it is
				...
				String nodeExpl=nt.get(id);
				ps.println(node+" ;; "+n.id+" ;; "+nodeExpl);
				shownNodes.add(n.id);
				index++;
